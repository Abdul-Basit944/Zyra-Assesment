...
def _url_category_score(url: str):
    lower = url.lower()
...
def select_pages(url_scores: dict[str, int], seed_url: str):
    adm_candidates = []
    tui_candidates = []
...
def run(seed_url: str, name: str, extractor: UniversityExtractor):
    ...
    if not admissions_url:
        logger.warning(f"Could not identify admissions page for {name}")

    if not tuition_url:
        logger.warning(f"Could not identify tuition page for {name}")
...
def main():
    ...
    for name, url in universities.items():
        try:
            ...
            logger.info(f"Saved → output/{name}.json")
